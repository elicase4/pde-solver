...
std::array<int,2> pde::mesh::Mesh<2>::flat2idx(std::size_t flat_idx) const {
	std::array<int,2> out {};
	out[0] = static_cast<int>(flat_idx % N[1]);
	out[1] = static_cast<int>(flat_idx - out[0] * N[1]);
	return out;
}
...
std::array<int,3> pde::mesh::Mesh<3>::flat2idx(std::size_t flat_idx) const {
	std::array<int,3> out {};
	out[2] = static_cast<int>(flat_idx % N[0]);
	out[0] = static_cast<int>(flat_idx % N[1] - out[2] * N[0]);
	out[1] = static_cast<int>(flat_idx - out[1] * N[1] - out[2] * N[0] * N[1]);
	return out;
}
