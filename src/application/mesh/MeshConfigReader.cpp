...
namespace pdesolver {
	namespace application {
		namespace mesh {
			...
			MeshConfig MeshConfigReader::read(const std::string& filename) {
				...
				cfg.type = MeshConfigParser::parseMeshType(YAMLReader::required<std::string>(meshNode, "type"));

				if (cfg.type == MeshConfig::Type::Block2D) {
				...
				} else if (cfg.type == MeshConfig::Type::Gmsh) {
