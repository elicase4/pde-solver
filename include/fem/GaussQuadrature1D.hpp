...
namespace pdesolver {
	namespace fem {
		...
		template <>
		HOST_DEVICE void GaussQuadrature1D<1>::getPoints(Real* xi){
			xi[0] = 0.0;
		}

		template <>
		HOST_DEVICE void GaussQuadrature1D<1>::getWeights(Real* w){
			w[0] = 2.0;
		}
		
		// 2-point implementation
        template<>
        HOST_DEVICE void GaussQuadrature1D<2>::getPoints(Real* xi) {
            xi[0] = -0.5773502691896257;  // -1/sqrt(3)
            xi[1] =  0.5773502691896257;  //  1/sqrt(3)
        }

        template<>
        HOST_DEVICE void GaussQuadrature1D<2>::getWeights(Real* w) {
            w[0] = 1.0;
            w[1] = 1.0;
        }
        ...
        template<>
        HOST_DEVICE void GaussQuadrature1D<3>::getPoints(Real* xi) {
        ...
        template<>
        HOST_DEVICE void GaussQuadrature1D<3>::getWeights(Real* w) {
        ...
        template<>
        HOST_DEVICE void GaussQuadrature1D<4>::getPoints(Real* xi) {
        ...
        template<>
        HOST_DEVICE void GaussQuadrature1D<4>::getWeights(Real* w) {
        ...
        template<>
        HOST_DEVICE void GaussQuadrature1D<5>::getPoints(Real* xi) {
        ...
        template<>
        HOST_DEVICE void GaussQuadrature1D<5>::getWeights(Real* w) {
