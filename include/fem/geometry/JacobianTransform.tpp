...
template<Int SpatialDim, Int ParametricDim, Int NodesPerElement>
PDE_HOST PDE_DEVICE Real JacobianTransform<SpatialDim, ParametricDim, NodesPerElement>::computeMeasure(const Real detJ, const Real detg){
	
	Real measure;

	if constexpr (SpatialDim != ParametricDim){
		measure = std::sqrt(detg);
	} else {
		measure = detJ;
	}
...
template<Int SpatialDim, Int ParametricDim, Int NodesPerElement>
PDE_HOST PDE_DEVICE void JacobianTransform<SpatialDim, ParametricDim, NodesPerElement>::computeMetric(const Real* nodeCoords, const Real* dNdxi, const Real* J, Real* g){
	...
	if constexpr (ParametricDim == SpatialDim){
		...
		for (Index a = 0; a < NodesPerElement; ++a){
			for (Index i = 0; i < SpatialDim; ++i){
				for (Index beta = 0; beta < ParametricDim; ++beta){
					for (Index alpha = 0; alpha < ParametricDim; ++alpha){
						dNdx[a*SpatialDim + i] += (g[alpha*ParametricDim + beta] * J[i*ParametricDim + beta] * dNdxi[a*ParametricDim + alpha]);
					}
				}
			}
		}
